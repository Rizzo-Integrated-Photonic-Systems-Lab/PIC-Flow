...
class CombinedDataset(Dataset):
    """
    Dataset that merges multiple sweep directories (e.g., coupler_sweep and y_branch_sweep)
    that share the same grid size/resolution/channel layout.
    ...
    def __init__(
        ...
        super().__init__()
        self.roots = [Path(r) for r in root_dirs]
        self.split = split
        self.train_fraction = train_fraction
        self.normalize_eps = normalize_eps
        self.use_double = use_double
        ...
        if not all_dirs:
            raise RuntimeError(f"No sample subfolders found under {self.roots}")
