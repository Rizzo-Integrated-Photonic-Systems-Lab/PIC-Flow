# euler_bend.py
"""
90-degree Euler/adiabatic bend waveguide (2D effective-index)

Euler bends have gradually varying curvature (0 -> max -> 0),
which reduces mode mismatch and bending loss compared to circular bends.

Compatible with Device2DBase, supports:
...
def _rotate_points(points_xy, angle_deg):
...
def _rotate_vec(v, angle_deg):
    """Rotate a 2D vector by angle_deg about origin."""
    return _rotate_points(np.asarray(v, dtype=np.float64).reshape(1, 2), angle_deg)[0]


def euler_bend_points(theta_total_rad, R_min_um, n_pts=256, sign=1.0):
    """
    Generate points along an Euler spiral bend.

    Curvature profile: triangular ramp (0 -> kmax -> 0)
    - kmax = 1 / R_min_um (maximum curvature)
    - Total bend angle: theta = ∫ k(s) ds = 0.5 * kmax * L
    - Arc length: L = 2 * theta * R_min

    Args:
        theta_total_rad: total bend angle in radians (e.g., π/2 for 90°)
        R_min_um: minimum radius of curvature in microns
        n_pts: number of points along the curve
        sign: +1 for counter-clockwise, -1 for clockwise

    Returns:
        x, y, L: arrays of x and y coordinates, and total arc length
    """
    ...
    kmax = 1.0 / R
    L = 2.0 * theta / kmax  # Total arc length
...
# Device class
# -------------------------
class EulerBend2D(Device2DBase):
    """
    2D Euler (adiabatic) bend waveguide with two mode ports.

    Geometry:
      - Input lead (straight) along +x
      - Euler bend section (triangular curvature profile)
      - Output lead (straight) rotated by bend_angle_deg
      - Optional rotation_deg to rotate the whole device
    ...
    def build_geometry(self):
        ...
        # Build Euler bend centerline
        xb, yb, L_bend = euler_bend_points(
            theta_total_rad=self.bend_angle_rad,
            R_min_um=self.R_min_um,
            n_pts=192,
            sign=+1.0,  # Counter-clockwise
        )
        bend_pts = np.column_stack([xb, yb])
        ...
        if abs(self.rotation_deg) > 1e-9:
            bend_pts = _rotate_points(bend_pts, self.rotation_deg)
