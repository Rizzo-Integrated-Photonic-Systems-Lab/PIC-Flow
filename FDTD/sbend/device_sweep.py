...
from sbend import EulerSBend2D
...
def main():
    ...
    out_dir = Path(args.out_dir) if args.out_dir else (repo_root / "Data" / "sbend_sweep")
    sims_dir = out_dir / "sims"
    ensure_dir(sims_dir)

    u = latin_hypercube(args.n_geo, d=3, seed=args.seed)
    wg_widths = WG_WIDTH_MIN + u[:, 0] * (WG_WIDTH_MAX - WG_WIDTH_MIN)
    offsets = OFFSET_MIN + u[:, 1] * (OFFSET_MAX - OFFSET_MIN)
    rmins = RMIN_MIN + u[:, 2] * (RMIN_MAX - RMIN_MIN)
    wg_widths = quantize_01(wg_widths, WG_WIDTH_MIN, WG_WIDTH_MAX)
    offsets = quantize_01(offsets, OFFSET_MIN, OFFSET_MAX)
    ...
    for i in range(args.n_geo):
        geom_id = make_geom_id("sbend")
        input_port = int(rng.choice([1, 2]))

        params = {
            "wg_width_um": float(wg_widths[i]),
            "lateral_offset_um": float(offsets[i]),
