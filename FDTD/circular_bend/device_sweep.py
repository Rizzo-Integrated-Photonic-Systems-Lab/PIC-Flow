...
from circular_bend import CircularBend2D
...
def main():
    ...
    for i in range(args.n_geo):
        geom_id = make_geom_id("circular_bend")
        input_port = int(rng.choice([1, 2]))

        params = {
            "wg_width_um": float(wg_widths[i]),
            "bend_radius_um": float(radii[i]),
        }

        geom_rows.append({
            "geometry_id": geom_id,
            "device": "circular_bend",
            "split": splits[i],
