...
from straight import StraightWaveguide2D
...
LENGTH_MIN, LENGTH_MAX = 6.0, 16.0
...
def main():
    ...
    for i in range(args.n_geo):
        geom_id = make_geom_id("straight")
        input_port = int(rng.choice([1, 2]))

        params = {
            "wg_width_um": float(wg_widths[i]),
            "dev_length_um": float(lengths[i]),
        }
